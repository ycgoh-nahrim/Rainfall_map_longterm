...
# Summarise by decade
RF_decadal_avg <- RF_decadal %>% 
  group_by(Stn_no, Decade) %>% 
  summarise(Depth_avg = mean(Depth_yr))


# list decades
decade_list <- as.data.frame(unique(RF_decadal_avg$Decade))


# sort list
colnames(decade_list) <- "Decade"
# Extract the starting year
decade_list$start_year <- as.numeric(gsub("-.*", "", decade_list$Decade))
#order(decade_list$start_year, decreasing = F)
decade_list <- decade_list[order(decade_list$start_year, decreasing = F), ]
...
#############################
# INTERPOLATE FOR ALL DECADES

##  Use IDW

# decadal rainfall data join stn data

RF_data_stn <- RF_decadal_avg %>% 
  merge(RF_stn, by = "Stn_no")
...
for (i in 1:(nrow(decade_list))) {
  #i=1
  sel_yr <- decade_list[i, 1]
  
  #filter according to date
  RF_data_decadal <- RF_data_stn %>% 
    filter(Decade == sel_yr)
  
  # convert to sf
  sf_rf_decadal <- st_as_sf(RF_data_decadal, coords = c('Long', 'Lat'), crs = 4326)
  
  
  ## Nearest Neighbour
#  fit_NN <- gstat::gstat( # using package {gstat} 
#    formula = Depth_avg ~ 1,    # The column  we are interested in
#    data = as(sf_rf_decadal, "Spatial"), # using {sf} and converting to {sp}, which is expected
#    nmax = 10, nmin = 3 # Number of neighboring observations used for the fit
#  )
...
#  decadal_NN_mask <- mask(decadal_NN, mask = sel_shp2)
  ...
  #counter <- counter + 1
  interp_list[[i]] <- decadal_IDW_mask
...
max(RF_decadal_avg$Depth_avg)
min(RF_decadal_avg$Depth_avg)
...
col_brk <- c(1200, 1500, 1750, 2000, 2250, 2500, 2750, 3000, 3500, 4000)
